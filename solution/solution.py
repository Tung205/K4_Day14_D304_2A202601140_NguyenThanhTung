...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "Evaluate the answer against the rubric and return a JSON object with scores (0.0-1.0) and reasoning."
        )
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_s = min(scores)
        if min_s == failure.faithfulness:
            return "Context is missing or irrelevant — improve retrieval"
        elif min_s == failure.relevance:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            sfix = suggestions[i] if i < len(suggestions) else (suggestions[0] if suggestions else "Review pipeline")
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0 or any(f.relevance < 0.5 for f in failures):
            suggestions.append("Refine prompt clarity and query intent classification")
